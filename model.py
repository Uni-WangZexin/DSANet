...
class DSANet(LightningModule):
    ...
    def loss(self, labels, predictions):
        if self._hparams.criterion == 'l1_loss':
            loss = F.l1_loss(predictions, labels)
        elif self._hparams.criterion == 'mse_loss':
    ...
    def configure_optimizers(self):
        ...
        """
        optimizer = optim.Adam(self.parameters(), lr=self._hparams.learning_rate)
    ...
    def mydataloader(self, train):
        ...
        set_type = train
        print('MTSFzhiqian',set_type)
        dataset = MTSFDataset(window=self._hparams.window, horizon=self._hparams.horizon, data_name=self._hparams.data_name, set_type=set_type, data_dir=self._hparams.data_dir)
        print('MTSFzhihou',set_type)
    ...
    def add_model_specific_args(parent_parser, root_dir): # pragma: no cover
        ...
        parser.add_argument('--data_name', default='electricity',type=str)
        parser.add_argument('--data_dir', default='./data/multivariate-time-series-data/', type=str)
